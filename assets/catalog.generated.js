// 自动生成，请运行 node scripts/build-catalog.js；不要手动编辑。
window.EMBEDDED_CATALOG = {
  "version": 1,
  "categories": [
    {
      "id": "electronics",
      "path": "electronics/index.html",
      "url": "electronics/index.html",
      "title": "电学基础",
      "desc": "从电压、电流和电阻建立电路直觉。",
      "icon": "V",
      "order": 1,
      "lessons": [
        {
          "path": "electronics/voltage.html",
          "url": "electronics/voltage.html",
          "title": "电压",
          "desc": "电压与电势差交互实验",
          "order": 10
        },
        {
          "path": "electronics/current.html",
          "url": "electronics/current.html",
          "title": "电流",
          "desc": "电流与欧姆定律交互实验",
          "order": 20
        },
        {
          "path": "electronics/resistor.html",
          "url": "electronics/resistor.html",
          "title": "电阻",
          "desc": "电阻与功率交互实验",
          "order": 30
        }
      ],
      "no": "01"
    },
    {
      "id": "transistor",
      "path": "transistor/index.html",
      "url": "transistor/index.html",
      "title": "晶体管",
      "desc": "观察 NPN、PNP 与 MOSFET 的工作区。",
      "icon": "Q",
      "order": 2,
      "lessons": [
        {
          "path": "transistor/npn.html",
          "url": "transistor/npn.html",
          "title": "NPN 三极管",
          "desc": "NPN三极管工作区交互实验",
          "order": 10
        },
        {
          "path": "transistor/pnp.html",
          "url": "transistor/pnp.html",
          "title": "PNP 三极管",
          "desc": "PNP三极管工作区交互实验",
          "order": 20
        },
        {
          "path": "transistor/mosfet.html",
          "url": "transistor/mosfet.html",
          "title": "MOSFET",
          "desc": "MOSFET工作区交互实验",
          "order": 30
        }
      ],
      "no": "02"
    },
    {
      "id": "c-language",
      "path": "c-language/index.html",
      "url": "c-language/index.html",
      "title": "C 语言",
      "desc": "用二进制理解求余与位运算。",
      "icon": "C",
      "order": 3,
      "lessons": [
        {
          "path": "c-language/binary.html",
          "url": "c-language/binary.html",
          "title": "二进制",
          "desc": "8位二进制交互实验",
          "order": 10
        },
        {
          "path": "c-language/modulo.html",
          "url": "c-language/modulo.html",
          "title": "求余运算",
          "desc": "C语言求余运算交互实验",
          "order": 20
        },
        {
          "path": "c-language/bitwise.html",
          "url": "c-language/bitwise.html",
          "title": "位运算",
          "desc": "C语言位运算交互实验",
          "order": 30
        }
      ],
      "no": "03"
    },
    {
      "id": "8051",
      "path": "8051/index.html",
      "url": "8051/index.html",
      "title": "51 单片机",
      "desc": "GPIO、定时器与 UART 的动态推演。",
      "icon": "51",
      "order": 4,
      "lessons": [
        {
          "path": "8051/gpio.html",
          "url": "8051/gpio.html",
          "title": "GPIO",
          "desc": "51单片机GPIO端口交互实验",
          "order": 10
        },
        {
          "path": "8051/timer0.html",
          "url": "8051/timer0.html",
          "title": "Timer0",
          "desc": "51单片机Timer0初值计算",
          "order": 20
        },
        {
          "path": "8051/timer2.html",
          "url": "8051/timer2.html",
          "title": "Timer2",
          "desc": "51单片机Timer2溢出交互实验",
          "order": 30
        },
        {
          "path": "8051/uart.html",
          "url": "8051/uart.html",
          "title": "UART Mode 1",
          "desc": "UART Mode 1与Timer2波特率交互实验",
          "order": 40
        }
      ],
      "no": "04"
    },
    {
      "id": "stm32",
      "path": "stm32/index.html",
      "url": "stm32/index.html",
      "title": "STM32",
      "desc": "从寄存器到 HAL 的进阶路线。",
      "icon": "S",
      "order": 5,
      "lessons": [],
      "no": "05"
    },
    {
      "id": "freertos",
      "path": "freertos/index.html",
      "url": "freertos/index.html",
      "title": "FreeRTOS",
      "desc": "任务、调度、队列与同步。",
      "icon": "RT",
      "order": 6,
      "lessons": [],
      "no": "06"
    },
    {
      "id": "pcb",
      "path": "pcb/index.html",
      "url": "pcb/index.html",
      "title": "PCB / AD",
      "desc": "从原理图到 PCB 输出的完整流程。",
      "icon": "P",
      "order": 7,
      "lessons": [],
      "no": "07"
    }
  ]
};
